...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/request-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/request-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/request-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make request-service-restart && make nginx-restart \
                        && make request-service-logs-without-tail \
